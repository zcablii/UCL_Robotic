...
class GeneralForwardSearchAlgorithm(PlannerBase):
    ...
    def search(self, startCoords, goalCoords):
        ...
        while (self.isQueueEmpty() == False):
            ...
            if rospy.is_shutdown():
                return False
            self.max_len = self.getLength() if self.getLength()>self.max_len else self.max_len
    ...
    def calculateAngleTurned(self, pathEndCell):
        total = 0
        current = pathEndCell
        next = pathEndCell.parent
        currentDirection = 0
        prevDirection = self.calculateDirection(current,next)
        while(next is not None):
            currentDirection = self.calculateDirection(current, next)
            total+=abs(currentDirection-prevDirection)
    ...
    def extractPathEndingAtCell(self, pathEndCell, colour):
        ...
        path.totalAngle = self.calculateAngleTurned(pathEndCell)
        ...
        print "Max queue length = " + str(self.max_len)
        print "Angle Robot turn = " + str(path.totalAngle)
